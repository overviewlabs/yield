...
The iOS app starts a server-created, single-use authorization and opens the validated provider-approved setup destination in the Safari app, where the user signs in to Robinhood and approves the connection. A token-free, pairing-bound custom-scheme callback returns the user to Yield, which re-fetches canonical connection status. It does not request Robinhood credentials, embed Robinhood in a webview, or receive or store OAuth codes, broker tokens, or MCP credentials. The non-Demo route remains unavailable until Treasury Agent's client registration, exact callback, and isolated connector are configured and approved.
