...
The iOS app starts a server-created, single-use authorization and opens the validated provider-approved setup destination in `ASWebAuthenticationSession`, where the user signs in to Robinhood and approves the connection. It accepts a token-free app return, re-fetches canonical connection status, and keeps QR, Copy, and Share available as optional browser-retry methods. It does not request Robinhood credentials, embed Robinhood in a webview, or receive or store OAuth codes, broker tokens, or MCP credentials. The non-Demo route remains unavailable until Treasury Agent's client registration, exact callback, and isolated connector are configured and approved.
