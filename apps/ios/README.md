...
The iOS app starts a server-created, single-use authorization and opens a pre-addressed email draft containing the validated, short-lived provider destination. The user sends the draft to their verified account email and opens it on a desktop computer, while Yield retains the pairing and polls canonical server status. It does not request Robinhood credentials, embed or proxy Robinhood, or receive or store OAuth codes, broker tokens, or MCP credentials.
