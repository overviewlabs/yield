...
Tapping **Connect to Robinhood** once creates or resumes a short-lived server pairing, requests the authorization transaction for that pairing, validates the returned destination in both the API and app, and opens the exact server-provided Robinhood OAuth URL in the Safari app. After authorization, the registered `yield://broker-connection/callback` URL returns the user to Yield; the app validates the result and pairing ID before re-fetching canonical server status. The app does not embed a `WKWebView` or impose a desktop-only restriction. The public `https://agent.robinhood.com/mcp/trading` address is a Streamable HTTP MCP resource for registered clients, not a browser setup page, so the app never opens it directly as onboarding.
