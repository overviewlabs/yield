...
Tapping **Email Desktop Link** creates or resumes a short-lived server pairing and asks the WHOX API to send the authorization URL to the Robinhood email entered by the user. The server applies that normalized address as the provider's OAuth login hint, validates the authorization destination, sends it through its SMTP transport, and returns only masked delivery status and expiry to the phone. The user opens the message on a desktop computer, where Robinhood supports the authorization page. Yield keeps the pairing pending and re-fetches canonical server status while active and whenever the app returns to the foreground; the email contains no broker credential. The app does not embed or proxy Robinhood. The public `https://agent.robinhood.com/mcp/trading` address is a Streamable HTTP MCP resource for registered clients, not a browser setup page, so the app never opens it directly as onboarding.
