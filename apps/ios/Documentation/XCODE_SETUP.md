...
5. For device or archive builds, set the Apple Developer Team in `project.yml`, regenerate the project, and create matching App IDs for `ai.whox.metis` and `ai.whox.metis.widget`.

The signing account must provision:

- Sign in with Apple for the app target.
- App Attest/DeviceCheck for the app target.
- App Group `group.ai.whox.metis` for both app and widget.
