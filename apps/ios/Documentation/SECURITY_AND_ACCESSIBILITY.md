...
- Mobile authorization accepts only an HTTPS `robinhood.com` or boundary-safe subdomain URL, the registered `yield` callback scheme, an exact return path, a matching pairing ID, and a token-free result. Unknown, duplicate, sensitive, fragmented, or user-info callback fields fail closed.
- The exact short-lived desktop authorization may be emailed again until it expires. Explicit cancel or regeneration invokes the authenticated invalidation boundary; a reset that cannot be confirmed causes the pairing to be canceled and replaced.
