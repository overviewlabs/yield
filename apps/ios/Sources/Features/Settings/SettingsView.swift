  ...
  var body: some View {
    List {
      Section("Classification") {
        ...
        Text(
          "This classification explains strategy eligibility inside WHOX Treasury and is not brokerage options approval."
        )
      }
  ...
  var body: some View {
    List {
      ...
      Section("Retention and processors") {
        Text(
          "Production disclosures must list actual processors and explain regulatory retention. WHOX Treasury never claims deletion of records that law requires it to retain."
        )
      }
      Section("Controls") {
      ...
      Section {
        Button("Copy Diagnostic Summary") {
          UIPasteboard.general.string =
            "WHOX Treasury | mode=\(session.mode.title) | connection=\(session.connection.status.title) | app=1.0"
  ...
  var body: some View {
    NavigationStack {
      Form {
        Section {
          DisclosureNotice(
            title: "Delete WHOX Treasury account",
            message:
