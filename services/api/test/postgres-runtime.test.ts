...
describe("PostgreSQL Paper API runtime",{skip:databaseUrl===undefined},()=>{
  it("survives restart, enforces tenant RLS, rejects refresh replay, and consumes pairing material once",async()=>{
    ...
    try{
      ...
      const mobileAuthorization=await pairings.beginMobileAuthorization(owner.userId,creator.sessionId,mobilePairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(instant.getTime()+96_000).toISOString());
      ...
      const abortedAuthorization=await pairings.beginMobileAuthorization(owner.userId,creator.sessionId,mobilePairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(instant.getTime()+99_000).toISOString());
      ...
      const callbackClaimedAuthorization=await pairings.beginMobileAuthorization(owner.userId,creator.sessionId,mobilePairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(instant.getTime()+99_010).toISOString());
      ...
      const revokedCallbackAuthorization=await pairings.beginMobileAuthorization(owner.userId,revokedCallbackSession.sessionId,revokedCallbackPairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(instant.getTime()+99_102).toISOString());
      ...
      const sensitiveStart={authorizationUrl:`https://auth.broker.test/authorize?state=${revokedCallbackAuthorization.state}`,callbackScheme:"whoxtreasury",returnUrl:"whoxtreasury://broker-connection/callback",pairingId:mobilePairing.pairingId,expiresAt:new Date(instant.getTime()+300_000).toISOString()};
  ...
  it("serves persisted Paper onboarding, entitlements, settings, portfolio, agents, approvals, orders, and notifications without Demo fixtures",async()=>{
    ...
    try{
      ...
      const verifiedSubscription={productID:"whox.treasury.equity.monthly",transactionID:`transaction-${suffix}`,originalTransactionID:`original-${suffix}`,environment:"Sandbox" as const,signedPayloadDigest:"c".repeat(64),appAccountToken:account.userId,purchasedAt:"2026-08-01T13:59:00.000Z",expiresAt:"2026-09-01T14:00:00.000Z",signedAt:"2026-08-01T14:00:00.000Z"};
      ...
      await store.syncVerifiedSubscription(draftAccount.userId,{...verifiedSubscription,productID:"whox.treasury.equitypro.monthly",transactionID:`draft-transaction-${suffix}`,originalTransactionID:`draft-original-${suffix}`,signedPayloadDigest:"e".repeat(64),appAccountToken:draftAccount.userId});
      ...
      await database.withTenant(account.userId,async(transaction)=>{
        ...
        await transaction.query(`INSERT INTO notifications(id,user_id,notification_type,priority,title,private_body,status,idempotency_key,scheduled_at) VALUES($1,$2,'proposal_ready','normal','Paper proposal ready','Open WHOX Treasury to review.','queued',$3,$4)`,[ids.notification,account.userId,`notification-${suffix}`,now]);
  ...
  it("revokes every session, device, APNs token, broker binding, pairing, and queued sync when an account is closed",async()=>{
    ...
    try{
      ...
      const completedAuthorization=await pairings.beginMobileAuthorization(account.userId,session.sessionId,completedPairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(closeInstant.getTime()+1).toISOString());
      ...
      const danglingAuthorization=await pairings.beginMobileAuthorization(account.userId,session.sessionId,danglingPairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(closeInstant.getTime()+4).toISOString());
  ...
  it("keeps PostgreSQL replacement blocked until the durable provider revocation is acknowledged",async()=>{
    ...
    try{
      ...
      const authorization=await pairings.beginMobileAuthorization(account.userId,session.sessionId,pairing.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(authorizationInstant.getTime()+1).toISOString());
      ...
      assert.equal((await pairings.beginMobileAuthorization(account.userId,session.sessionId,replacement.pairingId,"https://api.whox.test/v1/brokers/robinhood/mobile-oauth/callback","whoxtreasury://broker-connection/callback",new Date(authorizationInstant.getTime()+13).toISOString())).resumed,false);
    }finally{await database.close();}
  });
});
