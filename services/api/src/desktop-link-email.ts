  ...
  public async send(message: DesktopLinkEmail): Promise<void> {
    ...
    try {
      await this.#transport.sendMail({
        ...
        text: `Open this single-use link on your desktop computer:\n\n${link.href}\n\nThe Robinhood email field is prefilled for ${recipient}. This link expires ${expires}. If you did not request it, do not open it. Yield never asks for your Robinhood password by email.`,
        html: `<p>Open this single-use link on your desktop computer:</p><p><a href="${escapeHtml(link.href)}">Connect Robinhood to Yield</a></p><p>The Robinhood email field is prefilled for <strong>${escapeHtml(recipient)}</strong>. This link expires ${escapeHtml(expires)}.</p><p>If you did not request it, do not open it. Yield never asks for your Robinhood password by email.</p>`,
      });
    } catch {
